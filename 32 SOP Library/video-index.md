# Video Index

> Status: **Populated** — 12 SOPs indexed across Real Estate, SaaS, and E-Commerce. All videos currently pending recording.

## Purpose

Single source of truth for the video-walkthrough coverage of this portfolio, per the requirement defined in [`49 Internal Standards`](../49%20Internal%20Standards/README.md#8-video-walkthroughs). Every SOP, workflow doc, and case study must appear here exactly once, whether its video is recorded or still pending.

## Coverage Summary

| Metric | Count |
|---|---|
| Total documented automations | 12 |
| Videos recorded | 0 |
| Videos pending | 12 |
| Coverage % | 0% (documentation coverage 100%, recording coverage 0%) |

## Index

| Doc Type | Section | SOP / Doc Title | Video Status | Link | Duration |
|---|---|---|---|---|---|
| SOP | 07 Real Estate | [RE-01: Speed-to-Lead Response & Drip Nurture Engine](../07%20Real%20Estate/RE-01%20Speed-to-Lead%20Response%20and%20Drip%20Nurture%20Engine/SOP.md) | Pending | — | — |
| SOP | 07 Real Estate | [RE-02: Transaction Coordination & Compliance Automation](../07%20Real%20Estate/RE-02%20Transaction%20Coordination%20and%20Compliance%20Automation/SOP.md) | Pending | — | — |
| SOP | 07 Real Estate | [RE-03: AI-Powered Lead Qualification & Scoring Engine](../07%20Real%20Estate/RE-03%20AI-Powered%20Lead%20Qualification%20and%20Scoring%20Engine/SOP.md) | Pending | — | — |
| SOP | 07 Real Estate | [RE-04: CRE Deal Pipeline & Comp Analysis Automation](../07%20Real%20Estate/RE-04%20CRE%20Deal%20Pipeline%20and%20Comp%20Analysis%20Automation/SOP.md) | Pending | — | — |
| SOP | 14 SaaS | [SAAS-01: Trial-to-Paid Conversion & Usage Nurture Engine](../14%20SaaS/SAAS-01%20Trial-to-Paid%20Conversion%20and%20Usage%20Nurture%20Engine/SOP.md) | Pending | — | — |
| SOP | 14 SaaS | [SAAS-02: Automated Dunning & Failed-Payment Recovery Engine](../14%20SaaS/SAAS-02%20Automated%20Dunning%20and%20Failed-Payment%20Recovery%20Engine/SOP.md) | Pending | — | — |
| SOP | 14 SaaS | [SAAS-03: Churn Prediction & Proactive CS Intervention System](../14%20SaaS/SAAS-03%20Churn%20Prediction%20and%20Proactive%20CS%20Intervention%20System/SOP.md) | Pending | — | — |
| SOP | 14 SaaS | [SAAS-04: Usage-Based Billing Reconciliation & RevRec Pipeline](../14%20SaaS/SAAS-04%20Usage-Based%20Billing%20Reconciliation%20and%20RevRec%20Pipeline/SOP.md) | Pending | — | — |
| SOP | 15 E-Commerce | [ECOM-01: Multi-Market Product Content Governance](../15%20E-Commerce/ECOM-01%20Multi-Market%20Product%20Content%20Governance/SOP.md) | Pending | — | — |
| SOP | 15 E-Commerce | [ECOM-02: Review Intelligence & Response Queue](../15%20E-Commerce/ECOM-02%20Review%20Intelligence%20and%20Response%20Queue/SOP.md) | Pending | — | — |
| SOP | 15 E-Commerce | [ECOM-03: Catalog & Inventory Reconciliation](../15%20E-Commerce/ECOM-03%20Catalog%20Inventory%20Reconciliation/SOP.md) | Pending | — | — |
| SOP | 15 E-Commerce | [ECOM-04: Support Routing & SLA Control](../15%20E-Commerce/ECOM-04%20Support%20Routing%20and%20SLA%20Control/SOP.md) | Pending | — | — |

## How This File Is Maintained

1. When a new SOP, workflow doc, or case study is authored, add a row here immediately — even if the video is not yet recorded (`Video Status: Pending`).
2. When a video is recorded, update the row's `Video Status` to `Recorded`, add the link and duration, and update the Coverage Summary counts.
3. This file is regenerated/reconciled at the end of each staged build session against the actual documents present, so it never drifts from reality.

---
*Part of the Enterprise Automation Portfolio. See root [README.md](../README.md) for navigation.*
